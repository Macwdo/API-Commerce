...
@router.get("/{id_us}/produtos/{id_pd}",tags=["Produtos"])
async def get_id_user_prod(id_us: int,id_pd: int):
    vend = {"vendedor":id_us}
    produto = await Produto.objects.get_or_none(id=id_pd,**vend)
    if produto is None:
        raise HTTPException(
            status.HTTP_404_NOT_FOUND
        )
...
@router.patch("/{id_us}/produtos/{id_pd}",response_model=ProdutoResponse,tags=["Produtos"])
async def update_patch(id_us: int,id_pd: int, prod_data: ProdutoPatch,user: UsuarioSCHM = Depends(get_current_user),response: Response = Response()):
    produto_id = await Produto.objects.get_or_none(id=id_pd)
    id_vendedor = dict(produto_id.vendedor)
    if permission(user,'admin') or id_us == int(id_vendedor.get("id")) and id_us == user.id:
...
@router.delete("/{id_us}/produtos/{id_pd}",tags=["Produto"])
async def delete(id_us:int,id_pd: int,user: UsuarioSCHM = Depends(get_current_user),response: Response = Response()):
    produto_id = await Produto.objects.get_or_none(id=id_pd)
    if not produto_id:
        raise HTTPException(
    ...
    id_vendedor = dict(produto_id.vendedor)
    if id_us == user.id or permission(user, 'admin') and int(id_vendedor.get("id")) == id_us:
